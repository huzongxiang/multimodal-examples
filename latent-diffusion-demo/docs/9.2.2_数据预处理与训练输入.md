...
训练阶段真正输入模型的，并不是原始图像和原始文本，而是经过预处理后的图像潜变量与文本嵌入。这个 Demo 的数据准备也因此不是简单的数据搬运，而是把原始图文对转换成单卡环境能够稳定消费的训练输入。训练代码里出现的 `latent` 和 `embedding`，正是这条路线的直接结果。
...
两条路线分别对应“从原始图文对自行预处理”和“直接使用已处理特征”。
...
这一部分对应项目中的 `data.py`。它负责从图文数据出发，完成下载、VAE 编码、CLIP 编码，并将结果保存为 HDF5 文件。
...
## 4. HDF5 到 NumPy 的转换脚本

这一部分对应补充的 `convert_hdf5_to_npy.py`。它的作用很简单：读取预处理产出的 HDF5 文件，并导出训练代码需要的 `.npy` 文件。
...
这一层虽然只是一个格式转换脚本，但在真实项目中非常重要。真正让完整流程成立的，往往不只是“大模型本身”，还包括这些负责连接不同阶段的中间工具。
...
- 当前案例更强调把重点放在模型训练与推理流程本身，而不是耗费在下载失败或数据源不稳定上。

因此，本次实践主要使用了 Hugging Face 上公开的 `small_ldt` 预处理特征，并在此基础上进行训练和采样。

## 6. 下载预处理特征的脚本

这一部分对应补充的 `download_small_ldt.py`。它通过 Hugging Face 官方接口下载预处理数据。
...
这一部分对应 `make_small_ldt_subset.py`。
...
- 数据格式衔接、特征下载和子集裁剪这些看似“工程性”的环节，实际上是让整个实践流程顺利建立起来的关键部分。

训练与推理代码中的核心问题，是模型如何读取这些预处理数据、完成去噪训练，并最终生成图像结果。
