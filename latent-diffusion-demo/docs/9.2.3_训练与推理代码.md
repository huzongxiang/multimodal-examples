...
一个 Latent Diffusion Demo 是否真正闭合，不取决于单个模型文件，而取决于训练主循环、训练入口和训练后采样脚本能否连成一条连续主线。当前项目正是通过这三部分完成从预处理特征读取、训练记录到训练后 prompt 采样的完整流程。
...
这一部分对应项目中的 `train.py`。它是整个流程的核心组织代码。虽然模型本身定义在前面的章节中，但真正让模型“开始学习”的，是这一层训练主循环。
...
把数据准备代码与训练代码放在一起看，就很容易理解一个完整项目的组织方式：预处理阶段负责生成训练所需的特征文件，训练阶段直接围绕这些特征展开，避免每一轮都重复运行外部编码器。

## 2. 面向单卡环境的训练入口

在技术写作中，直接暴露大量底层训练配置并不总是最合适的安排。更常见的做法，是提供一个面向实践的入口脚本，用更少的参数快速启动不同规模的训练。

这里，这一角色由 `train_demo_4070ti.py` 完成。它负责：
...
训练完成后，最直接的问题就是如何用训练得到的 `best checkpoint` 生成一批新的样本图像。这一部分由 `sample_best_ckpt_prompts.py` 完成。
